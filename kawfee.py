...
def get_db_connection():
    conn = sqlite3.connect("kawfee_sqlite_2025-11-19.db", check_same_thread=False)
...
kawfee_app = Flask(__name__)
kawfee_app.secret_key = "your_secret_key"

kawfee_app.config["WTF_CSRF_SECRET_KEY"] = "IeV86ag6-E6Y9gi4WMH68OmR2ExY2VO5KQPs8lSn5XA"

csrf = CSRFProtect(kawfee_app)

kawfee_app.config["PERMANENT_SESSION_LIFETIME"] = timedelta(days=31)

os.makedirs(os.path.join(kawfee_app.static_folder, "pfps"), exist_ok=True)
...
@kawfee_app.errorhandler(403)
def forbidden(e):
    return render_template("errors/403.html"), 403


@kawfee_app.errorhandler(400)
def handle_400(error):
    return render_template("errors/400.html"), 400


@kawfee_app.before_request
...
@kawfee_app.route("/admin/generate_invite", methods=["POST"])
def generate_invite():
    if not is_admin():
        abort(403)
...
@kawfee_app.route("/admin/invite_codes")
...
@kawfee_app.route("/accept_changes", methods=["POST"])
def accept_changes():
...
@kawfee_app.route("/filter/tag/<path:tag>")
...
@kawfee_app.route("/register", methods=["GET", "POST"])
def register():
...
@kawfee_app.route("/login", methods=["GET", "POST"])
def login():
    username = ""
    user = None
    admin_status = False
...
@kawfee_app.route("/logout")
def logout():
    session.pop("username", None)
    return redirect(url_for("index"))


@kawfee_app.route("/delete_fic/<int:fic_id>", methods=["POST"])
...
@kawfee_app.route("/add_tag", methods=["POST"])
def add_tag():
...
@kawfee_app.route("/profile/<username>")
...
@kawfee_app.route("/edit_profile", methods=["GET", "POST"])
def edit_profile():
    if "username" not in session:
        return redirect(url_for("login"))
    ...
    if request.method == "POST":
        ...
        if pfp_file and pfp_file.filename != "":
            ...
            upload_dir = os.path.join(kawfee_app.static_folder, "pfps")
...
@kawfee_app.route("/submit", methods=["GET", "POST"])
def submit():
    if not logged_in():
        return redirect(url_for("login"))
...
@kawfee_app.route("/edit/<int:fid>", methods=["GET", "POST"])
def edit_fic(fid):
...
@kawfee_app.route("/kudo/<int:fid>", methods=["POST"])
def add_kudo(fid):
    if not logged_in():
        return redirect(url_for("login"))
...
@kawfee_app.route("/comment/<int:fid>", methods=["POST"])
def add_comment(fid):
    if not logged_in():
        return redirect(url_for("login"))
...
@kawfee_app.route("/delete_comment/<int:fid>/<int:comment_index>", methods=["POST"])
...
@kawfee_app.route("/admin", methods=["GET", "POST"])
def admin_panel():
    ...
    # fetch fanfics, if kawfee_applicable
    fanfics = get_fanfics()
...
@kawfee_app.route("/admin/delete_user/<username>", methods=["POST"])
...
@kawfee_app.route("/admin/delete_fic/<int:fic_id>", methods=["POST"])
def delete_fic_admin(fic_id):
    ...
    try:
        ...
        # delete fanfic
        conn.execute("DELETE FROM fanfics WHERE id = ?", (fic_id,))
        # also delete associated chapters, tags, etc., if kawfee_applicable
...
@kawfee_app.route("/ban_ip", methods=["POST"])
def ban_ip():
    if not is_admin():
        abort(403)
...
@kawfee_app.route("/admin/set_display_name/<username>", methods=["GET", "POST"])
...
@kawfee_app.route("/notes/<note_id>")
...
@kawfee_app.route("/notes/new", methods=["GET", "POST"])
def new_note():
    if not logged_in():
        return redirect(url_for("login"))
...
@kawfee_app.route("/notes/<note_id>/edit", methods=["GET", "POST"])
...
@kawfee_app.route("/blog/new", methods=["GET", "POST"])
def new_blog_post():
    if not logged_in():
        return redirect(url_for("login"))
...
@kawfee_app.route("/blog/<post_id>/edit", methods=["GET", "POST"])
...
@kawfee_app.route("/blog/<post_id>/delete", methods=["POST"])
...
@kawfee_app.route("/admin/site-info", methods=["GET", "POST"])
def edit_site_info():
    # connect to the database
    conn = get_db_connection()
    cursor = conn.cursor()
...
@kawfee_app.template_filter("format_datetime")
def format_datetime(value):
    dt = datetime.fromisoformat(value)
    return dt.strftime("%Y-%m-%d %H:%M:%S")


@kawfee_app.route("/admin/logs")
def show_logs():
    username = session.get("username")
    # connect to the database
    conn = get_db_connection()
...
@kawfee_app.route("/admin/all_logs")
def all_logs():
    per_page = 20
...
@kawfee_app.route("/admin/logs/<username>")
...
@kawfee_app.template_filter("nl2br")
def nl2br_filter(s):
    if s is None:
        return ""
    return s.replace("\n", "<br>\n")
...
if __name__ == "__main__":
    kawfee_app.run(debug=True, host="0.0.0.0", port=5000)
